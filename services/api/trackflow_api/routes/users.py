...
def create_user(payload: UserCreate) -> UserPublic:
    try:
        ...
        db.close()
        raise HTTPException(status_code=500, detail="Error interno al crear usuario") from error
    db.close()
...
def list_users(_: UserRecord = Depends(require_admin)) -> list[UserPublic]:
    try:
        ...
        db.close()
        raise HTTPException(status_code=500, detail="Error interno al listar usuarios") from error
    db.close()
...
def get_user(user_id: str, current_user: UserRecord = Depends(get_current_user)) -> UserPublic:
    ...
    try:
        ...
        db.close()
        raise HTTPException(status_code=500, detail="Error interno al obtener usuario") from error
    db.close()
...
def update_user(
    ...
    if payload.role is not None and current_user.role != UserRole.ADMIN:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Solo administradores pueden cambiar roles")
    ...
    try:
        ...
        db.close()
        raise HTTPException(status_code=500, detail="Error interno al actualizar usuario") from error
    db.close()
...
def delete_user(user_id: str, current_user: UserRecord = Depends(get_current_user)) -> dict[str, str]:
    ...
    try:
        ...
        db.close()
        raise HTTPException(status_code=500, detail="Error interno al eliminar usuario") from error
    db.close()
